...
test('1. check contra servidor local descobre versao nova', async () => {
  ...
  const srv = await makeServer({ version: '0.3.0', installerContent: content });
  try {
    const svc = new UpdateService({ client: makeClient(srv.url) });
    const res = await svc.check(true);
    assert.equal(res.ok, true);
    assert.equal(res.latestVersion, '0.3.0');
...
test('2. download baixa, valida checksum e persiste', async () => {
  ...
  try {
    ...
    assert.equal(res.fileName, 'master-code-setup-0.3.0.exe');
    ...
    const expected = path.join(configDir(), 'updates', 'master-code-setup-0.3.0.exe');
    ...
    assert.equal(st.downloaded!.version, '0.3.0');
    assert.equal(st.downloaded!.fileName, 'master-code-setup-0.3.0.exe');
  } finally {
    await srv.close();
  }
});
...
test('3. checksum invalido aborta e remove o arquivo', async () => {
  ...
  const srv = await makeServer({ version: '0.3.0', installerContent: content, checksumLine: `${badHash}  master-code-setup-0.3.0.exe` });
  try {
    const svc = new UpdateService({ client: makeClient(srv.url) });
    const res = await svc.download();
    ...
    const target = path.join(configDir(), 'updates', 'master-code-setup-0.3.0.exe');
...
test('4. checksum ausente do sums aborta com mensagem de integridade', async () => {
  ...
  const srv = await makeServer({ version: '0.3.0', installerContent: content, checksumLine: `${sha256Hex(content)}  outro-arquivo.exe` });
  try {
    const svc = new UpdateService({ client: makeClient(srv.url) });
    const res = await svc.download();
    assert.equal(res.ok, false);
